...
//        checkOK();
	...
	private void init() {
		((KApplication) getApplication()).initSqlHelper();
...
//		}
	...
	private void initUi() {
		...
		prepareDialog = new ProgressDialog(this);
		prepareDialog.setTitle(null);
		prepareDialog.setMessage(getResources().getString(R.string.loading));
		prepareDialog.show();
		new InitService(this, new InitCallback() {
			@Override
			public void onInitFinished() {
				prepareDialog.cancel();
				Intent intent = new Intent().setClass(LoginActivity.this, HomeActivity.class);
				startActivity(intent);
				finish();
			}
		}).execute();
	...
	private void initUi(int flag) {
		...
		if (flag == 0) {
			...
			intent.setClass(this, com.king.khcareer.home.v6.ManagerActivity.class);
		}
		else if (flag == 2) {
