 ...
 * 描述:
 * <p/>作者：景阳
 * <p/>创建时间: 2017/7/3 11:22
 */
public class RecordFilterDialog extends CustomDialog implements CompoundButton.OnCheckedChangeListener {
    ...
    protected View getCustomView() {
        ...
        View view = LayoutInflater.from(getContext()).inflate(R.layout.dialog_record_filter, null);
        initView(view);
        return view;
    }
        ...
        if (v == saveButton) {
            if (cbox_sql.isChecked()) {
                ...
                if (actionListener != null) {
                    actionListener.onSave(new RecordService().queryByWhere(where, values));
                }
            } else {
                orgnizeData();
                if (actionListener != null) {
                    ...
                    List<Record> list = searchModel.searchFromList((List<Record>) map.get("data"));
                    actionListener.onSave(list);
                }
            }
        } else if (v.getId() == R.id.button_date_start) {
